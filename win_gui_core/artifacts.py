...
class ArtifactManager:
    ...
    def create_bundle(
        ...
        *,
        ...
        event_logs: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        ...
        screenshot_target: str | None = None
        ui_tree_path: str | None = None
        qt_state_path: str | None = None
        process_tree_path: str | None = None
        event_logs_path: str | None = None

        if session.last_screenshot_path:
            target = bundle_dir / "last-screenshot.png"
            shutil.copy2(session.last_screenshot_path, target )
            screenshot_target = str(target)
        ...
        if ui_tree is not None:
            target = bundle_dir / "uia-tree.json"
            target.write_text(json.dumps(ui_tree, indent=2), encoding="utf-8")
            ui_tree_path = str(target)
            session.ui_tree_path = ui_tree_path

        if qt_state is not None:
            target = bundle_dir / "qt-state.json"
            target.write_text(json.dumps(qt_state, indent=2), encoding="utf-8")
            qt_state_path = str(target)

        if process_tree is not None:
            target = bundle_dir / "process-tree.json"
            target.write_text(json.dumps(process_tree, indent=2), encoding="utf-8")
            process_tree_path = str(target)

        if event_logs and event_logs.get("path"):
            source_path = Path(str(event_logs["path"]))
            if source_path.exists():
                target = bundle_dir / "event-logs.txt"
                shutil.copy2(source_path, target)
                event_logs_path = str(target)
        ...
        manifest = {
            ...
            "last_screenshot_path": screenshot_target,
            "logs": copied_logs,
            "dumps": dumps,
            "ui_tree_path": ui_tree_path,
            "qt_state_path": qt_state_path,
            "process_tree_path": process_tree_path,
            "event_logs_path": event_logs_path,
